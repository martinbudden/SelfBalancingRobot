...
class BlackboxMessageQueue : public MessageQueueBase {
public:
    const AHRS::ahrs_data_t& getQueueItem() const { return _queueItem; };
#if defined(FRAMEWORK_USE_FREERTOS)
    ...
    //inline bool SEND_IF_NOT_FULL(const AHRS::ahrs_data_t& queueItem) const { (void)queueItem; return false; } // cppcheck-suppress knownConditionTrueFalse
#endif // USE_FREERTOS
private:
