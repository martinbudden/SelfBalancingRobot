...
Motors4EncoderMotor::Motors4EncoderMotor(float encoderStepsPerRevolution) :
...
void Motors4EncoderMotor::setPower(float leftPower, float rightPower)
{
    ...
    const int8_t leftOutput =   static_cast<int8_t>(round(leftClipped * MAX_POWER)); // NOLINT(hicpp-use-auto,modernize-use-auto)
    const int8_t rightOutput = -static_cast<int8_t>(round(rightClipped * MAX_POWER));
