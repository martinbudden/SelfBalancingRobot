...
int main([[maybe_unused]] int argc, [[maybe_unused]] char **argv)
{
    ...
    while (true) { 
        const xyz_t gyroRadians = {0.0, 0.0, 0.0}; // replace this with actual gyroscope data in radians/s
        const xyz_t accelerometer = {0.0, 0.0, 1.0}; // replace this with actual accelerometer data in g

        const Quaternion orientation = sensorFusionFilter.update(gyroRadians, accelerometer, deltaT);
